...
export default function HistoryView({ scriptFilter }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { setHistory(await fetchHistory()) } catch { /* ignore */ } finally { setLoading(false) }
  }, [])
...
"""
open('frontend/src/components/HistoryView.tsx', 'w').write(content)
print("Done: HistoryView rewritten")
