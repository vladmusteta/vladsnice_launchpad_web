  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-3 border-b border-slate-800 flex items-center gap-4">
        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">🖥️ Ansible Hosts Database</span>
            ...
            <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
              ...
              {inventories.map(inv => (
                <button key={inv.id} onClick={() => void handleLoadInventory(inv.id)}
                  ...
                  <div className="font-medium truncate">{inv.is_ephemeral ? '🔄 ' : '📄 '}{inv.name}</div>
...
"""
open('frontend/src/components/AnsibleHostsPicker.tsx', 'w').write(content)
print("Done")
