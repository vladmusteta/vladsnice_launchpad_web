...
export default function LogsBrowser({ scriptFilter }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { setFiles(await fetchLogFiles()) } catch { /* ignore */ } finally { setLoading(false) }
  }, [])
...
"""
open('frontend/src/components/LogsBrowser.tsx', 'w').write(content)
print("Done: LogsBrowser rewritten with fuzzy search")
